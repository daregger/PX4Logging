...
fac1 = 7;
fac2 = 3;
pos_gps_google(:,1:2) = pos_gps(:,1:2)/power(10,fac1);
pos_gps_google(:,3) = pos_gps(:,3)/power(10,fac2);
...
maskWhereNotZero = ((pos_gps_google(:,2) ~= 0 & pos_gps_google(:,1) ~= 0 ) & pos_gps_google(:,3) ~= 0);
lonTemp = pos_gps_google(maskWhereNotZero,2);
latTemp = pos_gps_google(maskWhereNotZero,1);
elvTemp = pos_gps_google(maskWhereNotZero,3) + hightOffset; % in order to see the lines above ground
